...
public class Board {
    ...
    public int manhattan() { 
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                manhattan += calcManhattan(i, j , blocks[i][j]);
            }
        }

        return manhattan;
    }
    private int calcManhattan(int i, int j, int block) {         
        if (block == SPACE) {
            return 0;
        }
        int ii = (block - 1) / N;
    ...
    private Board exchange(Index i1, Index i2) {
    ...
    private static void testCase1() {
        ...
        Board b2 = b1.exchange(firstIndex, firstIndex.goRight());
